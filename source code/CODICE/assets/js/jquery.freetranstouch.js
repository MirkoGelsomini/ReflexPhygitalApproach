(function ($) {
    ...
    function _init(sel, options) {
        ...
        // translate (aka move)
        container.bind('touchstart.freetrans', function (evt) {
            
            var data = sel.data('freetrans');
            var p = Point(evt.touches[0].pageX, evt.touches[0].pageY);

            if (container.hasClass("frozen") || container.hasClass("final")) {
                //STOPPPPPPP
                ...
                var drag = function (evt) {
                    console.log(evt);
                    data.x += evt.touches[0].pageX - p.x;
                    data.y += evt.touches[0].pageY - p.y;
                    p = Point(evt.touches[0].pageX, evt.touches[0].pageY);
                    _draw(sel, data);
                };
        ...
        // rotate
        rotator.bind('touchstart.freetrans', function (evt) {
            evt.stopPropagation();

            var data = sel.data('freetrans'),
                cen = _getBounds(data._p.divs.controls).center,
                pressang = Math.atan2(evt.touches[0].pageY - cen.y, evt.touches[0].pageX - cen.x) * 180 / Math.PI;
            rot = Number(data.angle);

            var drag = _noSelect(function (evt) {
                var ang = Math.atan2(evt.touches[0].pageY - cen.y, evt.touches[0].pageX - cen.x) * 180 / Math.PI,
                    d = rot + ang - pressang;
        ...
        // scale
        container.find('.ft-scaler2').bind('touchstart.freetrans', _ifLeft(_noSelect(function (evt) {
            evt.stopPropagation();
